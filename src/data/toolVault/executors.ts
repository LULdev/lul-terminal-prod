...
const EXECUTORS: Record<string, ToolExecutor> = {
  ...
  'iso-week': (i) => { const d=new Date(i||Date.now()); if(Number.isNaN(d.getTime()))return 'Invalid date'; return `ISO Week: ${isoWeek(d)}`; },
  ...
  'time-add-hours': (i, i2) => { const d=new Date(i); if(Number.isNaN(d.getTime()))return 'Invalid date'; d.setHours(d.getHours()+(parseInt(i2,10)||0)); return d.toISOString(); },
  'time-add-minutes': (i, i2) => { const d=new Date(i); if(Number.isNaN(d.getTime()))return 'Invalid date'; d.setMinutes(d.getMinutes()+(parseInt(i2,10)||0)); return d.toISOString(); },
  'time-format-de': (i) => { const d=new Date(i||Date.now()); return Number.isNaN(d.getTime())?'Invalid date':d.toLocaleString('de-DE'); },
  'time-format-us': (i) => { const d=new Date(i||Date.now()); return Number.isNaN(d.getTime())?'Invalid date':d.toLocaleString('en-US'); },
  'time-unix-now': () => String(Math.floor(Date.now()/1000)),
  'time-unix-to-iso': (i) => { const n=parseInt(i,10); if(!Number.isFinite(n))return 'Invalid unix'; return new Date(n*1000).toISOString(); },
  'time-iso-to-unix': (i) => { const t=new Date(i).getTime(); return Number.isFinite(t)?String(Math.floor(t/1000)):'Invalid date'; },
  'time-quarter': (i) => { const d=new Date(i||Date.now()); if(Number.isNaN(d.getTime()))return 'Invalid date'; return `Q${Math.floor(d.getMonth()/3)+1} ${d.getFullYear()}`; },
  'time-day-of-year': (i) => { const d=new Date(i||Date.now()); if(Number.isNaN(d.getTime()))return 'Invalid date'; const start=new Date(d.getFullYear(),0,0); return String(Math.floor((d.getTime()-start.getTime())/86400000)); },
  'time-weekend-check': (i) => { const d=new Date(i||Date.now()); if(Number.isNaN(d.getTime()))return 'Invalid date'; const day=d.getDay(); return day===0||day===6?'Weekend 🎉':'Weekday 💼'; },
