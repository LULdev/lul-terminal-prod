...
import { ACCENT_STYLES, DASHBOARD_MENU_ITEM, LAB_MENU_ITEMS, MAIN_MENU_ITEMS, TabId } from '../../config/menuItems';
  ...
  return (
    <nav className="w-[220px] bg-[#0a0c10] border-r border-slate-800/50 flex flex-col p-4 shrink-0 z-10 min-h-0 h-full" id="sidebar-rail">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-1.5 pr-1" id="sidebar-main-controls">
        ...
        <div className="text-[9px] text-slate-600 uppercase tracking-[1.5px] font-bold mt-3 mb-1 px-2 pt-2 border-t border-slate-800/60">
          Creative Labs
        </div>
        ...
        {LAB_MENU_ITEMS.map((item) => {
          ...
          const styles = ACCENT_STYLES[accent];
          const isActive = activeTab === item.id;
          const locked = !isLoggedIn || item.locked;
          return (
            <button
              key={item.id}
              onClick={() => onTabClick(item.id)}
              title={item.tagline}
              className={`flex items-center gap-3 px-3 py-2 rounded-md text-left transition-all text-[11px] ${
                isActive ? styles.active : styles.idle
              }`}
            >
              <span>{item.icon}</span>
              <span className="flex items-center gap-1.5 min-w-0 flex-1">
                ...
                {locked && (
                  <Lock
                    className={`w-3 h-3 shrink-0 ${item.locked ? 'text-amber-400/75' : 'text-slate-600'}`}
