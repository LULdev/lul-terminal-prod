...
export function normalizeUserRecord(u) {
  return {
    ...
    referralsCount: Math.max(0, Math.floor(Number(u.referralsCount) || 0)),
    imagesUploaded: Math.max(0, Math.floor(Number(u.imagesUploaded) || 0)),
    memesCreated: Math.max(0, Math.floor(Number(u.memesCreated) || 0)),
    pastesCreated: Math.max(0, Math.floor(Number(u.pastesCreated) || 0)),
    pasteViewsTotal: Math.max(0, Math.floor(Number(u.pasteViewsTotal) || 0)),
    lulCoins: u.lulCoins != null ? Math.max(0, Math.floor(Number(u.lulCoins) || 0)) : 1000,
    ...extractPublicGameStats(u),
    gameJackpotsWon: Math.max(0, Math.floor(Number(u.gameJackpotsWon) || 0)),
    gameTotalWon: Math.max(0, Math.floor(Number(u.gameTotalWon) || 0)),
    gameTotalLost: Math.max(0, Math.floor(Number(u.gameTotalLost) || 0)),
    gameRpsMoves: {
      rock: Math.max(0, Math.floor(Number(u.gameRpsMoves?.rock) || 0)),
      paper: Math.max(0, Math.floor(Number(u.gameRpsMoves?.paper) || 0)),
      scissors: Math.max(0, Math.floor(Number(u.gameRpsMoves?.scissors) || 0)),
    },
    ...
    abuseWarnings: Math.max(0, Math.floor(Number(u.abuseWarnings) || 0)),
    onlineMinutes: Math.max(0, Math.floor(Number(u.onlineMinutes) || 0)),
